...
#   book = xlrd.open_workbook( fileNameIn.encode('cp1251'), formatting_info=True)
    ...
    for sheetName in sheetNames :                                # Организую цикл по страницам
        log.info('-------------------  '+sheetName +'  ----------')
        if   sheetName.lower() == 'samsung'       : convert_sheet( book, sheetName)
        elif sheetName.lower() == 'lg'            : convert_sheet( book, sheetName)
        elif sheetName.lower() == 'nec'           : convert_sheet( book, sheetName)
        elif sheetName.lower() == 'benq'          : convert_sheet( book, sheetName)
        elif sheetName.lower() == 'sharp'         : convert_sheet( book, sheetName)
        elif sheetName.lower() == 'iiyama'        : convert_sheet( book, sheetName)
        elif sheetName.lower() == 'philips'       : convert_sheet( book, sheetName)
        elif sheetName.lower() == 'viewsonic'     : convert_sheet( book, sheetName)
        elif sheetName.lower() == 'panasonic'     : convert_sheet( book, sheetName)
        elif sheetName.lower() == 'проекторы panasonic': convert_sheet( book, sheetName)
